...
DriverConfig loadConfig(const std::string& config_file) {
	...
	// parse driver config
	tinyxml2::XMLElement* driver_xml = doc.FirstChildElement("saiFlexivDriverConfig");
	...
	if(!driver_xml->Attribute("serialNumber")) {
		throw runtime_error("No 'serialNumber' attribute found in driver config file: " +
							config_file);
	}
	...
	if(!driver_xml->Attribute("robotType")) {
		throw runtime_error("No 'robotType' attribute found in driver config file: " +
							config_file);
	}
	...
	if(robot_type_str == "Rizon4") {
		...
		config.robot_type = RobotType::RIZON_4_S;
	}
	else {
	...
	if(driver_xml->Attribute("forceSensorLinkName")) {
		config.force_sensor_link_name = driver_xml->Attribute("forceSensorLinkName");
	}
