...
void PeriodicTask(flexiv::rdk::Robot &robot,
  ...
  try {
    ...
    redis_client->setGetBatchRizon4NoGripper(set_get_batch_key_names, tau_cmd_array, MassMatrix,
                                      robot_joint_sensor_feedback, robot_wrist_ft_sensor_feedback);
    ...
    if (!initialized_torque_bias) {
      ...
      Eigen::VectorXd decoupled_holding_torques = MassMatrix * holding_torques;
      for (int i = 0; i < 7; ++i) {
...
int main(int argc, char **argv) {
  ...
  } else { // Change the name of keys for Rizon 4 robot to reflect lack of explicit force-torque sensor
    RAW_WRIST_FORCE_SENSED_KEY = redis_prefix +
                                 "sensors::" + driver_config.robot_name +
                                 "::wrist_estimated_ft::force_raw";
    RAW_WRIST_MOMENT_SENSED_KEY = redis_prefix +
                                  "sensors::" + driver_config.robot_name +
                                  "::wrist_estimated_ft::moment_raw";
    TCP_FORCE_SENSED_KEY = redis_prefix +
                           "sensors::" + driver_config.robot_name +
                           "::wrist_estimated_ft::tcp_force";
    TCP_MOMENT_SENSED_KEY = redis_prefix +
                            "sensors::" + driver_config.robot_name +
                            "::wrist_estimated_ft::tcp_moment";
  }
  ...
  try {
    ...
    // priority
    scheduler.AddTask(std::bind(PeriodicTask, std::ref(robot),std::ref(model),std::ref(redis_client)),
